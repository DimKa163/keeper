...
func GetState(ctx context.Context, db *sql.Tx, name string) (*core.SyncState, error) {
	var syncState core.SyncState
	if err := db.QueryRowContext(ctx, getStateByNameStmt, name).Scan(&syncState.ID, &syncState.LastSyncTime); err != nil {
		return nil, err
	}
...
func SaveState(ctx context.Context, db *sql.Tx, state *core.SyncState) error {
	if _, err := db.ExecContext(ctx, upsertStateStmt, state.ID, state.LastSyncTime); err != nil {
		return err
	}
	return nil
}
